...
# Minimal, non-intrusive CANet CrySyS tuning runner:
...
DATASET_DIR="${SCRIPT_DIR}/../data_raw/06_CrySyS_dataset"
MODEL_DIR="${SCRIPT_DIR}/../../models/CANET_CRYSYS"
RESULTS_DIR="${ROOT_DIR}/Results"
SELECTION_CONFIG="${SCRIPT_DIR}/crysys_signal_selection.json"
ID_CONFIG="${SCRIPT_DIR}/crysys_id_config.json"
CACHE_DIR="${CACHE_DIR_OVERRIDE:-${SCRIPT_DIR}/../data_raw/06_CrySyS_dataset/_canet_parsed_cache}"
RUNS_DIR="${RUNS_DIR_OVERRIDE:-${RESULTS_DIR}/crysys_minimal_grid_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$RUNS_DIR"
...
ONE_CLASS_POLICY="${ONE_CLASS_POLICY:-include}"
SIGNAL_MASK_PATH="${SIGNAL_MASK_PATH_OVERRIDE:-}"
...
if [[ ! -f "$SUMMARY_CSV" ]]; then
  printf "run_name,experiment_id,best_epoch,best_val_loss,lr,hidden_size,window_size,quantile,avg_tpr,avg_tnr,avg_f1,macro_auc,n_scored_files,n_two_class_files,n_one_class_files\n" > "$SUMMARY_CSV"
fi
...
echo "CrySyS cache dir: $CACHE_DIR"
...
extract_experiment_id() {
  local log_file="$1"
  local exp
  exp="$(grep -oE 'Saved weights: .*/CrySyS_[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2}_epoch[0-9]+' "$log_file" | tail -1 | sed -E 's|.*CrySyS_([0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2})_epoch[0-9]+|\1|')"
  if [[ -z "$exp" ]]; then
...
n_scored = int(j.get("n_scored_files", 0))
n_two = int(j.get("n_two_class_files", 0))
n_one = int(j.get("n_one_class_files", 0))
...
for cfg in "${CONFIGS[@]}"; do
  ...
  TRAIN_CMD=(
    "$VENV_PY" "${SCRIPT_DIR}/canet_crysys_train.py"
  ...
  if [[ -n "$SIGNAL_MASK_PATH" ]]; then
    TRAIN_CMD+=(--signal-mask-path "$SIGNAL_MASK_PATH")
  fi
  ...
  TEST_CMD=(
    "$VENV_PY" "${SCRIPT_DIR}/canet_crysys_test.py"
    --dataset-dir "$DATASET_DIR"
    --model-dir "$MODEL_DIR"
  ...
  if [[ -n "$SIGNAL_MASK_PATH" ]]; then
    TEST_CMD+=(--signal-mask-path "$SIGNAL_MASK_PATH")
  fi

  "${TEST_CMD[@]}" 2>&1 | tee "${RUN_DIR}/test.log"
  ...
  for q in "${QUANTILES[@]}"; do
    ...
    "$VENV_PY" "${SCRIPT_DIR}/canet_crysys_score.py" \
      --results-dir "$PRED_DIR" \
      --experiment-id "$EXP_ID" \
      --quantile "$q" \
      --one-class-policy "$ONE_CLASS_POLICY" \
      --out-csv "$OUT_CSV" \
      --out-json "$OUT_JSON" \
