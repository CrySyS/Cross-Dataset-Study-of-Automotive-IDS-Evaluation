...
def train_autoencoder(args, file_index, autoencoder, x_train_seq):
    ...
    checkpoint_path = f"{root_dir}/../artifacts/model_ckpts/{dataset_name}/autoendoer_canshield_{dataset_name}_{time_step}_{num_signals}_{sampling_period}.keras"
    keras_callbacks = [
        EarlyStopping(monitor='val_loss', patience=10, mode='min'),
        ModelCheckpoint(checkpoint_path, monitor='val_loss', save_best_only=True, mode='auto', verbose=1)
    ]
    ...
    # Auto-select the window with highest variance in the middle third of training
    # data so the visualization is not dominated by flat startup windows.
    n_windows = x_train_seq.shape[0]
    lo = max(0, n_windows // 3)
    hi = min(n_windows, 2 * n_windows // 3)
    candidate_stds = x_train_seq[lo:hi, :, :, 0].reshape(hi - lo, -1).std(axis=1)
    vis_idx = lo + int(np.argmax(candidate_stds))
    visualize_autoencoder(autoencoder, x_train_seq[vis_idx:vis_idx + 1], visualize_dir, True)
