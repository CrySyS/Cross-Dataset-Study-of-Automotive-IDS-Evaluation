...
from tensorflow.keras.layers import Conv2D, LeakyReLU, MaxPooling2D, UpSampling2D, ZeroPadding2D, Cropping2D, Input
...
def get_new_autoencoder(time_step, num_signals):
    ...
    autoencoder.add(ZeroPadding2D((2, 2)))
    autoencoder.add(Conv2D(32, (5,5), strides=(1, 1), padding='same'))
    autoencoder.add(LeakyReLU(negative_slope=0.2))
...
def get_autoencoder(args):
    ...
    # Search for existing model (.keras preferred, .h5 fallback)
    model_list = glob.glob(f"{root_dir}/../artifacts/models/{dataset_name}/autoendoer_canshield_{dataset_name}_{time_step}_{num_signals}_*.keras")
    ...
    if sp_best > 0:
        autoencoder = load_model(best_model_dir)
